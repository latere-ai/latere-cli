...
func TestMultipartUploadRejectsMalformedSession(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), "the upload session is malformed") {
...
func TestMultipartUploadRejectsNonPositiveSize(t *testing.T) {
	...
	for _, size := range []int64{0, -1} {
		...
		if err == nil || !strings.Contains(err.Error(), "an upload size is a positive number of bytes") {
