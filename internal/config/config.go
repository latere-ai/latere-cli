...
// package with no internal imports so api, upgrade, modelkey, and commands can
// all share one definition of where latere keeps its state, instead of each
// carrying a copy of the same XDG resolution.
package config
...
// upgrade and the sandbox tunnel's machine id treat "" as skip-persist.
func Dir() string {
