...
func TestArcaPutRejectsNonRegularSource(t *testing.T) {
	...
	for name, source := range sources {
		t.Run(name, func(t *testing.T) {
			...
			defer server.Close()
			_, _, err := execArca(t, server, "put", source, "files/destination")
...
func TestArcaPutAllowsSymlinkToRegularFile(t *testing.T) {
	root := t.TempDir()
	...
	defer server.Close()
	if _, _, err := execArca(t, server, "put", link); err != nil {
		t.Fatal(err)
	}
}
