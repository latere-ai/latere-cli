...
func TestArcaListOutputFailures(t *testing.T) {
	...
	for _, long := range []bool{false, true} {
		t.Run(fmt.Sprintf("long=%t", long), func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				requests.Add(1)
				if r.Method != http.MethodGet || r.URL.Path != "/v1/files/me/files" || !r.URL.Query().Has("list") || r.Header.Get("Authorization") != "Bearer synthetic-token" {
			...
			execute := func(out io.Writer) error {
				...
				args := []string{"arca", "ls", "--api-url", server.URL, "--token", "synthetic-token"}
				if long {
