...
// TestArcaRoundtripE2E drives the built binary through the whole file
// lifecycle against a stateful fake Arca: put → ls → get → rm →
...
// internal/arca and internal/commands.
func TestArcaRoundtripE2E(t *testing.T) {
	if testing.Short() {
	...
	run := func(args ...string) string {
		t.Helper()
		cmd := exec.Command("go", append([]string{"run", ".", "arca",
			"--api-url", srv.URL, "--token", "e2e-token"}, args...)...)
		cmd.Env = append(os.Environ(), "OTEL_SDK_DISABLED=true")
		out, err := cmd.CombinedOutput()
		if err != nil {
			t.Fatalf("latere arca %s failed: %v\n%s", strings.Join(args, " "), err, out)
		}
		return string(out)
	}
...
// fakeArca is a minimal stateful Arca: enough of the file plane for the
// roundtrip above.
...
func newFakeArca(t *testing.T) *fakeArca {
	return &fakeArca{
		t:           t,
		files:       map[string][]byte{},
		trash:       map[string][]byte{},
...
func (f *fakeArca) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	f.mu.Lock()
	defer f.mu.Unlock()
	p := r.URL.Path
	switch {
...
func (f *fakeArca) handleFile(w http.ResponseWriter, r *http.Request, path string) {
	q := r.URL.Query()
	switch r.Method {
	case http.MethodPut:
...
func (f *fakeArca) handleCreateUpload(w http.ResponseWriter, r *http.Request) {
	var req struct {
		Path string `json:"path"`
...
func (f *fakeArca) handleCompleteUpload(w http.ResponseWriter, r *http.Request) {
