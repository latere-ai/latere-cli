...
func TestArcaRejectsMethodChangingRedirectsE2E(t *testing.T) {
	if testing.Short() {
	...
	for _, operation := range []string{"put", "mv", "rm", "get"} {
		for _, status := range []int{301, 302, 303} {
			t.Run(operation+"/"+strconv.Itoa(status), func(t *testing.T) {
				...
				args, method := []string{"arca", operation}, http.MethodGet
				switch operation {
				case "put":
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					initial.Add(1)
					if r.Method != method || r.URL.Path != "/v1/files/me/files/test" || r.Header.Get("Authorization") != "Bearer test-arca" {
				...
				defer server.Close()
				args = append(args, "--api-url", server.URL, "--token", "test-arca")
