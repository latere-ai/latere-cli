...
func TestArcaMultipartDestinationE2E(t *testing.T) {
	if testing.Short() {
	...
	for _, tc := range []struct {
		...
		for _, format := range []string{"text", "json"} {
			t.Run(tc.name+"/"+format, func(t *testing.T) {
				...
				defer server.Close()
				args := []string{"arca", "put", source, tc.want, "--api-url", server.URL, "--token", "synthetic-token"}
				if format == "json" {
				...
				} else if exit, ok := errors.AsType[*exec.ExitError](err); !ok || exit.ExitCode() != 1 || !strings.Contains(diagnostic.String(), "names a destination other than the requested path") || strings.Contains(diagnostic.String(), "Uploaded") || out.Len() != 0 {
