...
func TestArcaRestoreReceiptE2E(t *testing.T) {
	if testing.Short() {
		...
		for _, format := range []string{"text", "json"} {
			t.Run(tc.name+"/"+format, func(t *testing.T) {
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					if r.Method != http.MethodPost || r.URL.Path != "/v1/files/me/files/item" || r.Header.Get("Authorization") != "Bearer synthetic-token" {
						t.Errorf("request=%s %s", r.Method, r.URL)
					}
					_, _ = io.WriteString(w, tc.body)
				}))
				defer server.Close()
				args := []string{"arca", "restore", "files/item", "--version", "3", "--api-url", server.URL, "--token", "synthetic-token"}
				if format == "json" {
