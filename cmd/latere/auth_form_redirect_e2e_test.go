	...
	for _, operation := range []string{"org", "logout"} {
		for _, tc := range []struct {
			...
			t.Run(operation+"/"+tc.name, func(t *testing.T) {
				...
				const before = `{"access_token":"old-auth","refresh_token":"old-refresh","client_id":"test-client"}`
				...
				case operation == "logout":
					if !errors.Is(readErr, os.ErrNotExist) {
						t.Errorf("logout retained credential: %s", filepath.Base(authPath))
					}
				case invalid:
					if readErr != nil || string(data) != before {
						t.Errorf("rejected org response changed credential: %s", filepath.Base(authPath))
					}
				default:
					var saved struct {
						AccessToken string `json:"access_token"`
					}
					if readErr != nil || json.Unmarshal(data, &saved) != nil || saved.AccessToken != "new-auth" {
						t.Errorf("valid org response did not update credential: %s", filepath.Base(authPath))
					}
				}
				wantRedirects := int32(0)
				if !invalid && tc.redirect != 0 {
					wantRedirects = 1
				}
				if initial.Load() != 1 || redirected.Load() != wantRedirects {
					t.Errorf("initial/redirect calls = %d/%d, want 1/%d", initial.Load(), redirected.Load(), wantRedirects)
				}
			})
		}
	}
