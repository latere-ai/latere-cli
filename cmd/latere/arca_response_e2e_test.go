...
func TestArcaCommandsRequireCompleteResponsesE2E(t *testing.T) {
	if testing.Short() {
		...
		for _, state := range []string{"complete", "whitespace", "short content length", "interrupted chunks", "extra JSON", "trailing garbage", "no content"} {
			...
			t.Run(operation+"/"+state, func(t *testing.T) {
				...
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					calls.Add(1)
					if r.Method != method || r.URL.Path != path || r.Header.Get("Authorization") != "Bearer test-arca" {
