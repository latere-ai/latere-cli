		...
		for _, format := range []string{"text", "json"} {
			for _, writable := range []string{"1", "0"} {
				t.Run(tc.provider+"/"+format+"/writable="+writable, func(t *testing.T) {
					...
					command.Env = append(os.Environ(), "LATERE_AUTH_TOKEN_FILE="+filepath.Join(dir, "absent-auth.json"), "XDG_CONFIG_HOME="+dir, "LATERE_NO_UPDATE_CHECK=1", "OTEL_SDK_DISABLED=true", "LATERE_TEST_DOWNLOAD_OUTPUT="+output, "LATERE_TEST_DOWNLOAD_WRITABLE="+writable)
