	...
	for _, operation := range []string{"get", "delete"} {
		for _, state := range []string{"complete", "whitespace", "short content length", "interrupted chunks", "extra JSON", "trailing garbage", "no content"} {
			...
			t.Run(operation+"/"+state, func(t *testing.T) {
				...
				command.Env = append(os.Environ(), "LATERE_CELLA_TOKEN=test-cella", "LATERE_AUTH_TOKEN_FILE="+filepath.Join(root, "auth-token.json"), "AUTH_URL="+server.URL, "LATERE_NO_UPDATE_CHECK=1", "OTEL_SDK_DISABLED=true", "XDG_CONFIG_HOME="+root)
