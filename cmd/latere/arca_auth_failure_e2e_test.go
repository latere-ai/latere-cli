...
func TestArcaDoesNotSubstituteCellaAfterAuthFailureE2E(t *testing.T) {
	if testing.Short() {
	...
	for _, kind := range []string{"git helper", "file command"} {
		// wantRoot is the root token auth must see on the Arca mint; wantBearer
		// is what Arca must then receive. A usable root always yields the
		// minted token; only a pasted login reaches Arca verbatim.
		for _, tc := range []struct {
			...
			{name: "healthy auth", wantRoot: "root-access", wantBearer: "arca-actor"},
			{name: "refreshed auth", wantRoot: "new-root", wantBearer: "arca-actor", refreshStatus: 200},
			...
			t.Run(kind+"/"+tc.name, func(t *testing.T) {
				...
				var refreshes, mints, arcaCalls atomic.Int32
				server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					w.Header().Set("Content-Type", "application/json")
					if r.URL.Path == "/actor-tokens" {
						mints.Add(1)
						if tc.wantRoot == "" || r.Header.Get("Authorization") != "Bearer "+tc.wantRoot {
							t.Error("the Arca mint presented a substituted credential after auth failed")
						}
						_, _ = w.Write([]byte(`{"actor_token":"arca-actor","expires_in":300}`))
						return
					}
					if r.URL.Path == "/token" {
						refreshes.Add(1)
					...
					if tc.wantBearer == "" || r.Header.Get("Authorization") != "Bearer "+tc.wantBearer {
						t.Error("Arca received a substituted credential after auth failed")
					}
				...
				command.Env = append(os.Environ(), "LATERE_CELLA_TOKEN=", "LATERE_AUTH_TOKEN_FILE="+authPath, "AUTH_URL="+server.URL, "ARCA_API_URL="+server.URL, "LATERE_ARCA_TOKEN=", "AUTH_CLIENT_ID=", "LATERE_NO_UPDATE_CHECK=1", "OTEL_SDK_DISABLED=true", "XDG_CONFIG_HOME="+root)
				...
				if kind == "file command" && tc.wantBearer == "" {
					...
					if exit, ok := errors.AsType[*exec.ExitError](err); !ok || exit.ExitCode() != 1 || !strings.Contains(stderr.String(), want) {
						t.Errorf("failed Arca auth = %v; stderr: %s", err, stderr.String())
					}
				} else if err != nil {
				...
				var wantArca, wantRefresh, wantMint int32
				if kind == "file command" && tc.wantBearer != "" {
					wantArca = 1
				}
				if tc.refreshStatus != 0 {
				...
				if arcaCalls.Load() != wantArca || refreshes.Load() != wantRefresh || mints.Load() != wantMint {
					t.Errorf("requests: Arca=%d refresh=%d mint=%d, want %d/%d/%d", arcaCalls.Load(), refreshes.Load(), mints.Load(), wantArca, wantRefresh, wantMint)
				}
			})
		}
	}
