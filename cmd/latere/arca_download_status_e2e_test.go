...
func TestArcaDownloadRequiresCompleteStatusE2E(t *testing.T) {
	if testing.Short() {
		...
		for _, dest := range []string{"file", "stdout"} {
			for _, tc := range []struct {
				...
				t.Run(fmt.Sprintf("%s/%s/redirect=%t", tc.name, dest, redirect), func(t *testing.T) {
					...
					defer cancel()
					command := exec.CommandContext(ctx, binary, "arca", "get", "files/item", "-o", target, "--api-url", server.URL, "--token", "synthetic-token")
