...
func TestArcaListOutputFailureE2E(t *testing.T) {
	if testing.Short() {
			...
			for _, mode := range []string{"writable", "read-only", "transient"} {
				...
				t.Run(name, func(t *testing.T) {
					...
					server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						requests.Add(1)
						if r.Method != http.MethodGet || r.URL.Path != "/v1/files/me/files" || r.Header.Get("Authorization") != "Bearer synthetic-token" {
					...
					defer cancel()
					args := []string{"-test.run=^TestArcaListOutputHelperProcess$", "--", "arca", "ls", "--api-url", server.URL, "--token", "synthetic-token"}
					if long {
...
func TestArcaListOutputHelperProcess(t *testing.T) {
