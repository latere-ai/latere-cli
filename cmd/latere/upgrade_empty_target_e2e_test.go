...
func TestUpgradeEmptyTargetE2E(t *testing.T) {
	...
	for _, target := range []string{"", " ", "\t"} {
		t.Run(fmt.Sprintf("target=%q", target), func(t *testing.T) {
			...
			command.Env = append(os.Environ(), "LATERE_TEST_LATEST_TARGET="+target, "LATERE_TEST_LATEST_SERVER="+server.URL, "LATERE_NO_UPDATE_CHECK=1", "OTEL_SDK_DISABLED=true", "XDG_CONFIG_HOME="+dir, "LATERE_AUTH_TOKEN_FILE="+filepath.Join(dir, "absent-auth.json"))
