...
func TestArcaMultipartRequiresCompletePartResponseE2E(t *testing.T) {
	if testing.Short() {
	...
	for _, state := range []string{"empty", "body", "no content", "short content length", "interrupted chunks", "HTTP error"} {
		t.Run(state, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.URL.Path != "/part" && r.Header.Get("Authorization") != "Bearer test-arca" {
					t.Error("Arca control request lost authentication")
				}
				switch r.URL.Path {
				case "/v1/uploads":
			...
			defer cancel()
			command := exec.CommandContext(ctx, binary, "arca", "put", source, "files/test", "--api-url", server.URL, "--token", "test-arca")
