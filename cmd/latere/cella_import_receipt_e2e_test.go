...
func TestCellaImportReceiptE2E(t *testing.T) {
	if testing.Short() {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if err != nil || string(data) != tc.contents || part.FormName() != "tarball" || hdr.Name != "file" {
			...
			defer cancel()
			command := exec.CommandContext(ctx, binary, "cella", "import", "dev", "--input", source, "--api-url", server.URL)
			...
			err := command.Run()
			if tc.valid {
				if err != nil || !strings.Contains(stdout.String(), `"imported": "file"`) {
					t.Errorf("valid receipt = %v, stdout=%q, stderr=%q", err, stdout.String(), stderr.String())
				}
			} else {
				if exit, ok := errors.AsType[*exec.ExitError](err); !ok || exit.ExitCode() != 1 || !strings.Contains(stderr.String(), "import receipt") {
