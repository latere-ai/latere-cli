...
func TestArcaPaginationCyclesE2E(t *testing.T) {
	if testing.Short() {
		...
		for _, tc := range []struct {
			...
			for _, format := range []string{"text", "json"} {
				t.Run(strings.Join(verb, " ")+"/"+tc.name+"/"+format, func(t *testing.T) {
					...
					defer server.Close()
					args := append([]string{"arca", "--api-url", server.URL, "--token", "synthetic-token"}, verb...)
					if format == "json" {
					...
					} else if exit, ok := errors.AsType[*exec.ExitError](err); !ok || exit.ExitCode() != 1 || !strings.Contains(diagnostic.String(), "a cursor it already gave") || out.Len() != 0 {
