...
func TestArcaUploadReceiptE2E(t *testing.T) {
	if testing.Short() {
				...
				t.Run(mode+"/"+tc.name+"/"+format, func(t *testing.T) {
					...
					server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						...
						case r.URL.Path == "/v1/uploads":
							_ = json.NewEncoder(w).Encode(map[string]any{"id": "upload", "path": "files/item", "part_size": 16 << 20, "part_count": 2, "part_urls": []string{"http://" + r.Host + "/part", "http://" + r.Host + "/part"}})
					...
					defer server.Close()
					args := []string{"arca", "put", source, "files/item", "--api-url", server.URL, "--token", "synthetic-token"}
					if format == "json" {
