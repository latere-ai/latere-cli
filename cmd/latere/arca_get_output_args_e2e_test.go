...
func TestArcaGetOutputArgumentsE2E(t *testing.T) {
	if testing.Short() {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			defer cancel()
			args := append([]string{"arca", "--api-url", server.URL, "--token", "synthetic-token", "get", "files/report.txt"}, tc.args...)
			command := exec.CommandContext(ctx, binary, args...)
			command.Dir = dir
			command.Env = append(os.Environ(), "LATERE_NO_UPDATE_CHECK=1", "OTEL_SDK_DISABLED=true", "XDG_CONFIG_HOME="+filepath.Join(dir, "config"), "LATERE_AUTH_TOKEN_FILE="+filepath.Join(dir, "absent-auth.json"))
