...
development deployment or a test. `LATERE_LUX_TOKEN` and
`TOPOS_TOKEN` do the same for their products.
...
Without a saved login there is nothing to mint from. The git helper then
refuses with `not logged in; run `latere login``, and nothing is sent.
Run `latere login` to sign in again.
...
presented to Lux, a `sandboxd` one only to Cella, an `origo` one only to
git. Whichever token is on the
